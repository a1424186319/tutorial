...
# 需求：两个人分别做‘西蓝花’和‘红烧肉’，每个人都需要‘锅’和‘铲子’才可以炒菜，
# 只能一个人先做饭，第二个再做，下面用程序表达这个过程
...
"""
运行程序，结果卡死，两人都拿到了一个资源，但第二个资源都在对方手中，但由于自己的功能未完成，导致自己拿的资源没释放，两个线程并不聪明，不会把自己的资源先借给对方，而是双方一致等待对方。

避免这种情况发生，改为递归锁。threading.Lock()改为threading.RLock()
"""
"""
既然用户可以在代码中自由加锁控制，确保线程安全，那么为什么还有GIL？
因为GIL处于解释器层面，轮询查看有没有待销毁的垃圾，相当于执行某一项功能的线程。如果垃圾回收功能的线程跟代码功能的线程混在一起，容易出现数据冲突和安全问题，所以发明者分开设计了。
整体流程详见：线程整体示意图
"""