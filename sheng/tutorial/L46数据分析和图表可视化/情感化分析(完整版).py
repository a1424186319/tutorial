...
def get_comments():
    ...
    # 读表
    cursor = get_cursor()
    sql = """ SELECT content FROM jd order by creation_time desc limit 0, %s"""
...
def my_evaluate(postive_num=0,pos_degree_num=1,negertive_num=0,neg_degree_num=1):
    """
    我的评价
...
if __name__ == '__main__':
    ...
    positive_num,positive_num = count_positive_word(clean_word_list,comments_amount=len(comments))
...
"""
github中搜索关键词可以获取到相关字典(stopword.txt)
词性:
1.过滤掉'停用词','停止词'
2.搜索'情感词汇''积极词汇字典''消极词汇'
3.读取字典到变量
4.for循环分词后的结果,跟字典变量内容做对比.
"""
